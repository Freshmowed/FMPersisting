...
+ (NSDictionary *) columnDict;
{
    ...
    if ( ![columnNamesForKeysDict objectForKey: NSStringFromClass(self)] )
    {
        ...
        for (NSDictionary *bitFieldMapping in [self bitFieldMappings] )
        {
            NSString *bitFieldPropertyName = bitFieldMapping[@"bitFieldProperty"];
            
            if ( !bitFieldPropertyName )
            {
                NSLog(@"FMPersistingModel.columnDict: no 'bitFieldProperty' defined in bitFieldMapping.");
                continue;
            }
            
            NSString *bitFieldColumnName = aDict[bitFieldPropertyName];
            NSArray *bitFieldPropertyKeys = [bitFieldMapping allKeys];
            for (NSString *aKey in bitFieldPropertyKeys )
            {
                if ( [aKey isEqualToString: @"bitFieldProperty" ] ) continue;
                
                aDict[aKey] = bitFieldColumnName;
            }
        }
    }
...
+ (NSDictionary *) insertStatementWithValues: (NSDictionary *) aDict;
{
    ...
    return @{@"statement" : preString, @"valueArray" : columnValues, @"populatedStmt" : preStringPopulated };
}
...
{
    if (self.persistenceManager != ((FMPersistingModel *) anObject).persistenceManager) return NO;
...
{
    NSArray *bitFields = [[self class] bitFieldMappings];
    if ( bitFields && bitFields.count > 0 )
    {
        for (NSDictionary *bitFieldMapping in bitFields )
        {
            NSNumber *bitMask = bitFieldMapping[key];
            if ( !bitMask ) continue;
            
            NSString *bitFieldPropertyName = bitFieldMapping[@"bitFieldProperty"];
            if (!bitFieldPropertyName ) continue;
            
            
            BOOL boolValue = [self isBitSet: [bitMask unsignedIntegerValue] on: [self valueForKey: bitFieldPropertyName] ];
            return @(boolValue);
        }
    }
...
- (void) setValue: (id)value forUndefinedKey:(NSString *)key;
{
    ...
    if ( bitFields && bitFields.count > 0 )
    {
        for (NSDictionary *bitFieldMapping in bitFields )
        {
            ...
            numberValue = [self setBit: [bitMask unsignedIntegerValue] on: [self valueForKey: bitFieldPropertyName] to: [numberValue boolValue]];
            [self setValue: numberValue forKey: bitFieldPropertyName];
